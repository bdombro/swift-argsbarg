...
func completionZshInstallOrPrint(schema: CliCommand, printOnly: Bool) {
    ...
    guard let home = ProcessInfo.processInfo.environment["HOME"], !home.isEmpty else {
        FileHandle.standardError.write(
            Data("HOME is not set; use --print and redirect manually.\n".utf8))
        print(script, terminator: "")
        return
    }
    let dir = URL(fileURLWithPath: home).appendingPathComponent(".zsh/completions")
    do {
    ...
    var fname = "_" + schema.name
    fname = String(fname.map { $0 == "-" ? Character("_") : $0 })
    let outpath = dir.appendingPathComponent(fname)
    do {
        try script.write(to: outpath, atomically: true, encoding: .utf8)
    } catch {
        FileHandle.standardError.write(
            Data("Could not write \(outpath.path)\n".utf8))
        print(script, terminator: "")
        return
    }
    let okMsg =
